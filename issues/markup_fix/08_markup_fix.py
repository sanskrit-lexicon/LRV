"""
Markup fixer + audit for lrv.txt (LRV).
Counterpart of PWG#175, PWK#113, and siblings.

LRV-specific notes: 10 {#...#}<ab|ls> boundary collisions. No whitespace hits.

Inputs: ../../../csl-orig/v02/lrv/lrv.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "lrv" / "lrv.txt", HERE / "lrv.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "lrv_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(ls|i)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for lrv.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"LRV markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against lrv.txt.\n")
        f.write(f"Paired tags: <ls>, <i>\n")
        f.write("Notes: 10 {#...#}<ab|ls> boundary collisions. No whitespace hits.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <ls>, <i>\n\n")
